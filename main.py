...
parser.add_argument('--epoch', dest='epoch', type=int, default=12, help='# of epoch')
parser.add_argument('--batch_size', dest='batch_size', type=int, default=128, help='# images in batch')
parser.add_argument('--lr', dest='lr', type=float, default=0.001, help='initial learning rate for adam')
parser.add_argument('--use_gpu', dest='use_gpu', type=int, default=1, help='gpu flag, 1 for GPU and 0 for CPU')
parser.add_argument('--sigma', dest='sigma', type=float, default=1.5, help='noise level')
...
def main(_):
    ...
    if args.use_gpu:
        ...
        with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
            model = MODEL(sess, sigma=args.sigma, lr=args.lr, epoch=args.epoch, batch_size=args.batch_size,
